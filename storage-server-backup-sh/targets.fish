...
set R_SEC_MAILAPI_KEY _PLACEHOLDER_MAILKEY_
...
function email_notify # string -> int
    ...
    curl "https://recolic.net/api/email-notify.php?apiKey=$R_SEC_MAILAPI_KEY&recvaddr=root@recolic.net&b64Title="(echo "RECOLIC STORAGE SYSTEM NOTIFY" | base64 -w0)'&b64Content='(echo "[storage.recolic.net] $message" | base64 -w0) 1>&2
    return $status
end
