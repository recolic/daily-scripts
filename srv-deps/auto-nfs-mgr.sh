...
HMS_GOOD=0
HMS_BAD=1
# Function to check if hms.re is reachable
check_reachability() {
    if ping -c 1 hms.re &> /dev/null; then
...
while true; do
    ...
    if [ $stat != $prev_stat ]; then
        ...
        if [ $stat = $HMS_BAD ]; then
            ...
            mount -o bg,intr,soft,timeo=1,retrans=1,actimeo=1,retry=1 hms.re:/ /home/recolic/nfs
        fi
    fi
    prev_stat=$stat
