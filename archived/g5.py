...
def decode_alt(txn_bytes):
    ...
    print("Chain:", cid, chain_name(chain_id))
    print("Type:", tx_type_name(tx_type))
    print("Max total fee:", format((tf*gl)/1e18, ".8f"), "ETH")
    print("To:", hx(tx_to))
    print("Amount:", amt)
    print("Data:", hx(tx_data))
...
def decode_tx(raw_bytes, data_type):
    ...
    if data_type == 4:
        ...
        tx_type = 2
        decode_alt(txn_bytes)
        exit() # for known reason, next line will crash
