...
# Non-MS user just go with Ubuntu2204
[[ "$vmsize" = *v3 ]] && var_default_val vmimg Canonical:0001-com-ubuntu-pro-microsoft:pro-22_04:22.04.202405240 # gen1
var_default_val vmimg               Canonical:0001-com-ubuntu-pro-microsoft:pro-22_04-gen2:22.04.202405240

# Disable outbound Internet access if needed. (202504) (You need azcli older than 2.73.0)
var_default_val no_outbound_net     1
...
var_default_val plugin_src          https://git.recolic.net/root/daily-scripts/-/raw/master/srv-deps/azv-plugin-
...
# seperated from vnet_create_xtra_arg because 'no_outbound_net' only supported with --subnets. In this case, --subnet-prefixes would be ignored.
subnet_prefix_json=address-prefix:10.0.0.0/24
...
if [ "$vnet_ipv6" = 1 ]; then
    ...
    subnet_prefix_json="address-prefixes:[10.0.0.0/24,'fd00:db8:deca::/64']"
    explicit_vnet_create=1
fi

# This could also be a plugin.
if [ "$vnet_altaddr" != 0 ]; then
    [ "$vnet_ipv6" = 1 ] && echo "++ Error: vnet_altaddr conflicts with vnet_ipv6." && exit 1
    ...
    subnet_prefix_json="address-prefix:$vnet_iprange/24"
fi

# This could also be a plugin.
if [ "$no_outbound_net" = 1 ] ; then
    vnet_create_xtra_arg+=(--subnets "[{name:default,default-outbound-access:false,$subnet_prefix_json}]")
    explicit_vnet_create=1
fi

call_if_any plugin_before_vnet_creat
